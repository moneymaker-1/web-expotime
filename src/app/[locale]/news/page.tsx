        ...
        <section style={{ background: '#fff', paddingTop: '8rem', paddingBottom: '4rem', borderBottom: '1px solid #f0f0f0' }}>
          <div className="container-custom" style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: '#555', fontSize: '1.1rem', lineHeight: 1.7, maxWidth: 680, margin: '0 auto' }}>
              {isRtl ? 'آخر أخبار إكسبو تايم وصناعة المعارض' : 'Latest news from Expo Time and the exhibition industry'}
            </p>
          </div>
        </section>
        <section className="section-padding" style={{ backgroundColor: '#f8f8f8' }}>
          <div className="container-custom">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.75rem' }}>
              {news.map((item, i) => (
                <div key={i} style={{ background: '#fff', border: '1px solid #eee', borderRadius: '1rem', overflow: 'hidden' }}>
                  <div style={{ position: 'relative', aspectRatio: '16/9', background: '#f8f8f8' }}>
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    <span style={{ color: '#aaa', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.375rem', marginBottom: '0.75rem' }}>
                      <Calendar size={13} /> {item.date}
                    </span>
                    <h2 style={{ fontWeight: 700, color: '#1a1a1a', lineHeight: 1.4 }}>{isRtl ? item.titleAr : item.titleEn}</h2>
                  </div>
                </div>
              ))}
            </div>
