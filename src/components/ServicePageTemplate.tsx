...
export default function ServicePageTemplate({
  ...
  return (
    <>
      ...
      <main>
        {/* Hero */}
        <section style={{ background: 'linear-gradient(135deg, #0a1520 0%, #0f1e2d 100%)', paddingTop: '8rem', paddingBottom: '5rem', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
              {features.map((f, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '1.25rem', background: '#162534', borderRadius: '0.75rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <CheckCircle size={18} style={{ color: '#f3c716', flexShrink: 0, marginTop: '2px' }} />
        ...
        {/* CTA */}
        <section style={{ padding: '5rem 0', backgroundColor: '#0f1e2d' }}>
          <div className="container-custom" style={{ textAlign: 'center' }}>
            <div style={{ background: 'rgba(243,199,22,0.06)', border: '1px solid rgba(243,199,22,0.2)', borderRadius: '1.5rem', padding: '4rem 2rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link href={`/${locale}/contact`} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 2.5rem', background: 'linear-gradient(135deg, #f3c716, #d4a800)', color: '#0f1e2d', fontWeight: 800, fontSize: '1rem', borderRadius: '0.625rem', textDecoration: 'none' }}>
